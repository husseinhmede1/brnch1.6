...
public class BankFilesOutputService {
    ...
    public BankFilesOutputResponseDto saveBankFiles(final BankFilesOutputRequestDto bankFilesOutputRequestDto) {
        ...
        if (requestBankFilesOutput.isPresent()) { //Case of update
            ...
            savedBankFilesOutput = this.bankFilesOutputMapper.toEntity(bankFilesOutputRequestDto);
            savedBankFilesOutput.setCreatedBy(requestBankFilesOutput.get().getCreatedBy());
            savedBankFilesOutput.setCreatedDate(requestBankFilesOutput.get().getCreatedDate());
            savedBankFilesOutput.setUpdatedBy(Integer.valueOf(userDetails.getId()));
            savedBankFilesOutput.setUpdatedDate(new Timestamp(System.currentTimeMillis()));
        } else { //Case of create
            ...
            savedBankFilesOutput = this.bankFilesOutputMapper.toEntity(bankFilesOutputRequestDto);
            savedBankFilesOutput.setCreatedBy(Integer.valueOf(userDetails.getId()));
            savedBankFilesOutput.setCreatedDate(new Timestamp(System.currentTimeMillis()));
        }
        if (makerCheckerEngine.processIfRequired(bankFilesOutputRequestDto, this.getClass().getName(), new Object() {
        }
                .getClass()
                .getEnclosingMethod()
                .getName(), "")) {
        ...
        savedBankFilesOutput = this.bankFilesOutputRepository.save(savedBankFilesOutput);
    ...
    public List<BankFilesOutputResponseDto> mapOutputFileTemplateToBanks(OutputFileTemplateBankCodeMappingRequestDto outputFileTemplateBankCodeMappingRequestDto) {
        ...
        List<BankFilesOutput> saveBankFilesOutputList = new ArrayList<>();
        for (String bankCode : outputFileTemplateBankCodeMappingRequestDto.getBankCodes()) {
            ...
            saveBankFilesOutputList.add(saveBankFilesOutput);
        }
		if (makerCheckerEngine.processIfRequired(outputFileTemplateBankCodeMappingRequestDto, this.getClass().getName(), new Object() {
		}
				.getClass()
				.getEnclosingMethod()
				.getName(), "")) {
        ...
        for (BankFilesOutput bankFilesOutput : saveBankFilesOutputList) {
            BankFilesOutput savedBankFilesOutput = this.bankFilesOutputRepository.save(bankFilesOutput);
    ...
    public List<BankFilesOutputResponseDto> unMapOutputFileTemplateToBanks(OutputFileTemplateBankCodeMappingRequestDto outputFileTemplateBankCodeMappingRequestDto) {
        OutputFileTemplateHdr outputFileTemplateHdr = this.outputFileTemplateHdrRepository.findById(outputFileTemplateBankCodeMappingRequestDto.getOutputFileTemplateHdrId())
                .orElseThrow(() -> new BusinessException(ResponseCode.CFG_INVALID_OUTPUT_TEMPLATE_HDR_ID, HttpStatus.NOT_FOUND));
        List<BankFilesOutputResponseDto> listBankFilesOutputResponseDto = new ArrayList<BankFilesOutputResponseDto>();
		List<BankFilesOutput> deleteBankFilesOutputList = new ArrayList<>();
        for (String bankCode : outputFileTemplateBankCodeMappingRequestDto.getBankCodes()) {
            if (!this.bankFilesOutputRepository.existsByBankCodeAndOutputTemplateHdrId(bankCode, outputFileTemplateBankCodeMappingRequestDto.getOutputFileTemplateHdrId())) {
                continue;
            }
			...
			deleteBankFilesOutputList.add(bankFilesOutput);
        }
		if (makerCheckerEngine.processIfRequired(outputFileTemplateBankCodeMappingRequestDto, this.getClass().getName(), new Object() {
		}
				.getClass()
		...
		for (BankFilesOutput bankFilesOutput : deleteBankFilesOutputList) {
